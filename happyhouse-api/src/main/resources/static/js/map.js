...
// 지도에 최단경로를 표시합니다.
function displayPath(places) {
	picked = [];
	function calcSqrDistance(from, to) {
	...
	const distanceMatrix = new Array(places.length).fill(0)
		.map(() => new Array(places.length).fill());
	for (let i = 0; i < places.length; i++) {
		for (let j = i + 1; j < places.length; j++) {
			const distance = calcSqrDistance(places[i], places[j]);
	...
	visited.add(0);
	dfs(1, 0, [0], visited, distanceMatrix);
	console.log(picked);

	const linePath = picked.map((pick) => new kakao.maps.LatLng(places[pick].lat, places[pick].lng));
...
function dfs(index, sum, path, visited, distanceMatrix) {
	if (index === distanceMatrix.length) {
		if (minDistance > sum) {
	...
	for (let i = 1; i < distanceMatrix.length; i++) {
		...
		visited.add(i);
		path.push(i);
		dfs(index + 1, sum + distanceMatrix[path[index - 1]][i], path, visited, distanceMatrix);
		path.pop();
		visited.delete(i);
	}
}
